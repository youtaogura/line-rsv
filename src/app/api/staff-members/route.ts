...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name || typeof name !== "string" || name.trim().length === 0) {
      return createValidationErrorResponse({ name: "Name is required and must be a non-empty string" });
    }
    ...
    if (existingStaff) {
      return createErrorResponse("A staff member with this name already exists", 409);
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (duplicateError && duplicateError.code !== "PGRST116") {
      console.error("Error checking for duplicate staff member:", duplicateError);
      return createErrorResponse("Failed to check for conflicts");
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // 関連する予約があるかチェック
    const { data: relatedReservations, error: reservationError } = await supabase
      .from("reservations")
      .select("id")
    ...
    if (relatedReservations && relatedReservations.length > 0) {
      return createErrorResponse("Cannot delete staff member with existing reservations", 409);
    }
