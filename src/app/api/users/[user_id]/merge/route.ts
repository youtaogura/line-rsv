...
export async function POST(
  ...
  try {
    ...
    if (sourceUser.member_type !== 'guest') {
      return createValidationErrorResponse({
        member_type: 'Only guest users can be merged',
      });
    }
    ...
    if (targetUser.member_type !== 'regular') {
      return createValidationErrorResponse({
        member_type: 'Target user must be a regular member',
      });
    }
    ...
    if (sourceReservations && sourceReservations.length > 0) {
      ...
      if (conflictingReservations.length > 0) {
        return createErrorResponse(
          `Reservation time conflicts detected for the following times: ${conflictingReservations
    ...
    try {
      ...
      if (deleteSourceError) {
        ...
        return createErrorResponse(
          'Reservations transferred but failed to delete source user'
        );
      }
      ...
      if (fetchError) {
        ...
        return createErrorResponse(
          'Merge completed but failed to fetch updated user info'
        );
      }
