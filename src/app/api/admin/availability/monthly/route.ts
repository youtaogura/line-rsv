  ...
  try {
    ...
    if (!year || !month) {
      return createValidationErrorResponse({
        params: 'Year and month parameters are required',
      });
    }
    ...
    const { data: reservations, error: reservationsError } = await supabase
      .from('reservations')
      .select(
        'id, datetime, duration_minutes, reservation_menu_id, staff_member_id'
      )
      .eq('tenant_id', tenant.id)
    ...
    for (const staff of staffMembers as StaffMember[]) {
      const staffHours =
        staffBusinessHours?.filter((h) => h.staff_member_id === staff.id) || [];
      if (staffHours.length < 1) {
        continue;
      }
      const staffReservations =
        (reservations as Reservation[])?.filter(
          (r) => r.staff_member_id === staff.id
        ) || [];
    ...
    const response: MonthlyAvailability = {
      ...
      staffMembers: Array.from(staffTimeSlotsMap.entries()).map(
        ([staffId, timeSlots]) => ({
          id: staffId,
          timeSlots,
        })
