...
async function getUserInput() {
  ...
  let tenantName, adminUsername, adminPassword, adminName;
  ...
  while (true) {
    ...
    console.log("❌ Tenant name must be between 1 and 100 characters. Please try again.\n");
  }

  // 2. 管理者ユーザー名の入力
  while (true) {
    adminUsername = await askQuestion("👤 Enter admin username (3-50 characters): ");
    ...
    console.log("❌ Username must be between 3 and 50 characters. Please try again.\n");
  }
  ...
  // 4. 管理者表示名の入力
  while (true) {
    adminName = await askQuestion("📋 Enter admin display name (1-100 characters): ");
    if (adminName.length >= 1 && adminName.length <= 100) {
      break;
    }
    console.log("❌ Admin name must be between 1 and 100 characters. Please try again.\n");
  }

  return { tenantName, adminUsername, adminPassword, adminName };
}

async function createTenant() {
  try {
    const { tenantName, adminUsername, adminPassword, adminName } = await getUserInput();
    ...
    const confirm = await askQuestion("Do you want to create this tenant? (y/N): ");
